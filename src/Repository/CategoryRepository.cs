...
{
    #region READ
    public async Task<ResponseApi<List<dynamic>>> GetAllAsync(PaginationUtil<Category> pagination)
    {
        try
        {
    ...
    public async Task<int> GetCountDocumentsAsync(PaginationUtil<Category> pagination)
    {
        List<BsonDocument> pipeline = new()
        {
            new("$match", pagination.PipelineFilter),
