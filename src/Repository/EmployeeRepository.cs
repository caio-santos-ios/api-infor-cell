    ...
    public class EmployeeRepository(AppDbContext context) : IEmployeeRepository
{
    ...
    public async Task<ResponseApi<Employee?>> GetByIdAsync(string id)
    {
        try
        {
            Employee? address = await context.Employees.Find(x => x.Id == id && !x.Deleted).FirstOrDefaultAsync();
            return new(address);
        }
        catch
        {
    ...
    #region CREATE
    public async Task<ResponseApi<Employee?>> CreateAsync(Employee address)
    {
        try
        {
            await context.Employees.InsertOneAsync(address);
    ...
    #region UPDATE
    public async Task<ResponseApi<Employee?>> UpdateAsync(Employee address)
    {
        try
        {
            await context.Employees.ReplaceOneAsync(x => x.Id == address.Id, address);
    ...
    public async Task<ResponseApi<Employee>> DeleteAsync(string id)
    {
        try
        {
            ...
            await context.Employees.ReplaceOneAsync(x => x.Id == id, address);
