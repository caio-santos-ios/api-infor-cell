    ...
    public class PaymentMethodRepository(AppDbContext context) : IPaymentMethodRepository
{
    ...
    public async Task<ResponseApi<PaymentMethod?>> GetByIdAsync(string id)
    {
        try
        {
            PaymentMethod? address = await context.PaymentMethods.Find(x => x.Id == id && !x.Deleted).FirstOrDefaultAsync();
            return new(address);
        }
        catch
        {
    ...
    #region CREATE
    public async Task<ResponseApi<PaymentMethod?>> CreateAsync(PaymentMethod address)
    {
        try
        {
            await context.PaymentMethods.InsertOneAsync(address);
    ...
    #region UPDATE
    public async Task<ResponseApi<PaymentMethod?>> UpdateAsync(PaymentMethod address)
    {
        try
        {
            await context.PaymentMethods.ReplaceOneAsync(x => x.Id == address.Id, address);
    ...
    public async Task<ResponseApi<PaymentMethod>> DeleteAsync(string id)
    {
        try
        {
            ...
            await context.PaymentMethods.ReplaceOneAsync(x => x.Id == id, address);
